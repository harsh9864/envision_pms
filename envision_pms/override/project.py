        ...
        if template_task.get("depends_on") and not project_task.get("depends_on"):
            ...
            for child_task in template_task.get("depends_on"):
                if project_template_map and project_template_map.get(child_task.task):
                    ...
                    project_task.append(
                        "depends_on",
                        {"task": project_template_map.get(child_task.task).name},
                    )
                    project_task.save()
...
def hourly_reminder():
    ...
    for project in projects:
        if get_time(nowtime()) >= get_time(project.from_time) or get_time(
            nowtime()
        ) <= get_time(project.to_time):
...
def daily_reminder():
    ...
    for project in projects:
        if allow_to_make_project_update(
            project.name, project.get("daily_time_to_send"), "Daily"
        ):
            send_project_update_email_to_users(project.name)
...
def create_duplicate_project(prev_doc, project_name):
    ...
    task_list = frappe.get_all(
        "Task", filters={"project": prev_doc.get("name")}, fields=["name"]
    )
...
def get_users_email(doc):
    return [
        d.email for d in doc.users if frappe.db.get_value("User", d.user, "enabled")
    ]
